...
MAILCAT_PYTHON = "python"
BLACKBIRD_PYTHON = "python"
...
def _activity_from_smtp(result) -> dict:
...
async def run_cmd(cmd: list[str], cwd=None, timeout=180):
    print(f"[CMD] cwd={cwd}")
    print(f"[CMD] cmd={cmd}")
    ...
    try:
        ...
        print(f"[CMD] returncode={proc.returncode}")
...
async def run_mailcat(email: str):
    username = email.split("@")[0].strip()
    return await run_cmd(
        [MAILCAT_PYTHON, "mailcat.py", username],
        cwd=MAILCAT_DIR,
        timeout=220,
    )


async def run_blackbird(email: str):
    return await run_cmd(
        [BLACKBIRD_PYTHON, "blackbird.py", "-e", email],
        cwd=BLACKBIRD_DIR,
        timeout=220,
    )
...
def _build_site_row(item: dict) -> str:
    ...
    logo_html = (
        f'<a href="{html.escape(url)}" target="_blank" rel="noopener noreferrer" class="logo-link">'
        f'<img class="result-logo" src="{html.escape(logo)}" alt="{html.escape(title)}">'
        f'</a>'
        if url else
        f'<img class="result-logo" src="{html.escape(logo)}" alt="{html.escape(title)}">'
    )
    ...
    url_html = (
        f'<a class="result-link" href="{html.escape(url)}" target="_blank" rel="noopener noreferrer">{html.escape(domain)}</a>'
        if url else html.escape(domain)
    )
...
async def run_email_osint(email_value: str) -> dict:
    email_value = email_value.strip().lower()
    if not email_value or "@" not in email_value:
        raise ValueError("Нужен корректный email")

    holehe_res, mailcat_res, blackbird_res = await asyncio.gather(
        run_holehe(email_value),
        run_mailcat(email_value),
        run_blackbird(email_value),
    )

    smtp_result = await asyncio.to_thread(check_email_smtp, email_value)
    smtp_activity = _activity_from_smtp(smtp_result)

    username = _normalize_username(email_value)

    mailcat_items = parse_mailcat_output(mailcat_res["stdout"]) if mailcat_res["stdout"] else []
    holehe_info = parse_holehe_output(holehe_res["stdout"]) if holehe_res["stdout"] else {"checked": 0, "found": [], "errors": []}
    blackbird_info = parse_blackbird_output(blackbird_res["stdout"]) if blackbird_res["stdout"] else {"checked": 0, "duration": "", "found": []}

    return {
        "target_email": email_value,
        "username": username,
        "smtp": {
            "status": getattr(smtp_result, "status", "unknown"),
            "stage": getattr(smtp_result, "stage", ""),
            "mx_host": getattr(smtp_result, "mx_host", None),
            "smtp_code": getattr(smtp_result, "smtp_code", None),
            "smtp_message": getattr(smtp_result, "smtp_message", ""),
            "detail": getattr(smtp_result, "detail", ""),
            "activity": smtp_activity,
        },
        "mailcat": {
            "return_code": mailcat_res["returncode"],
            "items": mailcat_items,
            "raw": mailcat_res["stdout"],
            "stderr": mailcat_res["stderr"],
        },
        "holehe": {
            "return_code": holehe_res["returncode"],
            **holehe_info,
            "raw": holehe_res["stdout"],
            "stderr": holehe_res["stderr"],
        },
        "blackbird": {
            "return_code": blackbird_res["returncode"],
            **blackbird_info,
            "raw": blackbird_res["stdout"],
            "stderr": blackbird_res["stderr"],
        },
    }
...
def build_email_report_html(
    ...
    *,
    ...
    activity = smtp_data.get("activity", {
        "label": "не подтвержден",
        "color": "#f59e0b",
        "class_name": "status-unknown",
    })
    ...
    target_card = (
        '<div class="metric">'
        f'<div class="metric-title">Цель</div>'
        f'<div class="metric-value">{html.escape(target_email)}</div>'
        f'<div class="activity-wrap">'
        f'  <span class="activity-dot {html.escape(activity["class_name"])}"></span>'
        f'  <span class="activity-label" style="color:{html.escape(activity["color"])}">{html.escape(activity["label"])}</span>'
        f'</div>'
        '</div>'
    )
    ...
    html_report = template.format(
        ...
        smtp_status=html.escape(str(smtp_data.get("status", "unknown"))),
        smtp_stage=html.escape(str(smtp_data.get("stage", ""))),
        smtp_mx_host=html.escape(str(smtp_data.get("mx_host", "—") or "—")),
        smtp_code=html.escape(str(smtp_data.get("smtp_code", "—") or "—")),
        smtp_message=html.escape(str(smtp_data.get("smtp_message", "") or "—")),
        smtp_detail=html.escape(str(smtp_data.get("detail", "") or "—")),
        activity_label=html.escape(activity["label"]),
        activity_color=html.escape(activity["color"]),
        activity_class=html.escape(activity["class_name"]),
    )
